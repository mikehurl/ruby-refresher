...
def select_elements_starting_with_a(array)
  array.select { |item| item[0, 1] == "a" }
end

# keep only the elements that start with a vowel
def select_elements_starting_with_vowel(array)
  array.select { |item| item[0, 1] =~ /[aeiou]/ }
end
...
def word_count_a_file(file_path)
  ...
  words = []
  file.each_line { |line| words = line.scan(/\w+/) }
  words.length
end
...
def count_words_of_each_length_in_a_file(file_path)
  ...
  words.each { |word| word_length_count[word.length] += 1 }
  word_length_count
end
...
def fizzbuzz_without_modulo
  (1..100).each do |number|
    if number.divmod(15)[1].zero?
      puts "fizzbuzz"
    elsif number.divmod(5)[1].zero?
      puts "buzz"
    elsif number.divmod(3)[1].zero?
      puts "fizz"
    else
      puts number
    end
